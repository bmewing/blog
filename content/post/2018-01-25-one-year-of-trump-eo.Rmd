...
clinton %<>%
  mutate(signing_date = lubridate::as_date(signing_date,format="%m/%d/%Y",tz="EST")) %>% 
...
nixon %<>%
  mutate(start_date = lubridate::as_date("1969-01-20"),
         signing_date = lubridate::as_date(signing_date,format="%m/%d/%Y",tz="EST")) %>% 
...
orders %>% 
  ...
  layout(shapes=list(type='line', x0= 365, x1= 365, y0=0, y1=ymax, line=list(dash='dot', width=1)),
